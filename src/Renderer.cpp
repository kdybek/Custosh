...
namespace Custosh::Renderer
{
    namespace
    {
        ...
        {
            float w0 = cross2D(triangle2D.p0, triangle2D.p1, p);
            float w1 = cross2D(triangle2D.p1, triangle2D.p2, p);
            float w2 = cross2D(triangle2D.p2, triangle2D.p0, p);

            if (w0 == 0 && isBottomOrRight(triangle2D.p0, triangle2D.p1)) { return false; }
            if (w1 == 0 && isBottomOrRight(triangle2D.p1, triangle2D.p2)) { return false; }
            if (w2 == 0 && isBottomOrRight(triangle2D.p2, triangle2D.p0)) { return false; }
        ...
        {
            Vector4<float> pPerspective = Vector4(pm * p.normalizeW()).normalizeW();
        ...
        triangle2D_t applyPerspectiveTriangle(const triangle3D_t& triangle3D,
                                              const PerspectiveMatrix& pm)
        {
            return {.UUID = triangle3D.UUID,
                    .p0 = applyPerspectivePoint(triangle3D.p0, pm),
